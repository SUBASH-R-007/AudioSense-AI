// The pure-tone screening run, and nothing else.
//
// A screening run is a self-contained clinical procedure: instruct, anchor the
// scale, present tones in the clinical order, and hand back thresholds. Who the
// subject is, what is done with the numbers afterwards, and where the operator
// goes next are decisions for whoever is running the clinic that day — a camp
// screening a queue of walk-ins wants different follow-up from a consultation
// room. So this component measures and then gets out of the way: it collects no
// demographics, calls no analysis endpoint, and never navigates.
//
// Two things it will not compromise on, because both change what the numbers
// mean:
//
//   1. THE ANCHOR. Browser audio has no absolute reference. The operator sets
//      one by ear at 1 kHz, and every level afterwards is relative to it. That
//      is stated on the calibration screen, not only in the docs, because a
//      threshold quoted without its anchor invites being read as ISO 389 dB HL.
//
//   2. THE CATCH TRIALS. Silent presentations are interleaved throughout. A
//      response to silence never touches the threshold estimate; it goes to the
//      reliability monitor instead, and an unreliable run says so on the result
//      table rather than quietly reporting tidy-looking thresholds.
...
// The results table reads left-to-right in ascending frequency, which is how an
// audiogram is read, even though the tones are presented in clinical order.
const TABLE_FREQS = [250, 500, 1000, 2000, 4000, 8000]

/**
 * @param {(run: {right: Object, left: Object, reliability: Object, procedure: string}) => any} onComplete
 *   Called once when the operator accepts the finished run. May return a
 *   promise; the accept button stays busy until it settles, so a host that
 *   fails to save the run leaves the operator on the results with the thresholds
 *   still in front of them.
 * @param {() => void} [onCancel] Leave the measurement entirely. The escape
 *   hatch is only offered when a host provides one.
 * @param {string} [subjectLabel] Who is being tested, shown so the operator
 *   cannot screen the wrong person in a queue.
 */
// What the accept button does depends on who is hosting the run: the standalone
// screening page interprets the result immediately, while the pure-tone step
// hands the values to the threshold grid and carries on with the battery. A
// button that promised analysis in the second case would be describing
// something that does not happen.
export default function ScreeningRunner({
  onComplete, onCancel, subjectLabel,
  acceptLabel = 'Analyze my hearing →', busyLabel = 'Analyzing…',
}) {
  ...
  // A live AudioContext outlives the route unless it is closed, and a
  // calibration tone left running would follow the operator to the next screen.
  ...
  // Spacebar = "I hear it" (the standard patient response button). It is live
  // only while a presentation is awaiting an answer, so a key pressed between
  // tones cannot be scored as a response.
  useEffect(() => {
    const onKey = (e) => {
  ...
  // The run is handed over exactly once per acceptance; a second click while the
  // host is still working with it would submit the same thresholds twice.
  const accept = async () => {
      ...
      {stage === 'calibrate' && (
        <div className="mt-5 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-[13.5px] text-slate-600">
            ...
            anchors the scale at 40 dB HL. Every level after this is measured against
            that anchor, so the dB HL reported here is relative, not ISO 389 calibrated.
          </p>
